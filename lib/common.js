...
function writeAuthMarker(serverUrl, kind = "opened") {
  ...
  // millisecond; re-review P2).
  const claimId = newClaimId();
  try {
    fs.writeFileSync(
      tmpPath,
      JSON.stringify({ openedAt, kind, claimId }) + "\n"
    );
...
// The much smaller grace used when a "satisfied" judgment DELETES a marker
// (retireSatisfiedAuthMarker). A genuinely completed phase writes tokens
// strictly after its claim in real time, so only filesystem mtime
// granularity (2s on FAT) can invert the ordering — 5s covers that with
// slack. It must stay far below TOKENS_GRACE_MS: judging deletions with the
// 60s gate grace let any session retire a LIVE claim made within 60s of a
// token write and re-open the issue-#10 storm (Cubic P1, round 5). The
// residual is symmetric and bounded: a claim made within 5s of a token
// write can still be retired early (one extra tab), a completion whose
// mtime lies more than 5s behind the claim stays until the TTL.
const RETIRE_TOKENS_GRACE_MS = 5_000;
...
function tryRetireAuthMarker(serverUrl, ttlMs = AUTH_MARKER_TTL_MS) {
  return atomicRetirePath(
    ...
    (stolen) => {
      ...
      // NO TOKENS_GRACE_MS here (re-review P1): a contender's openedAt is
      // ≈now, so a graced check degrades to "was any token file touched in
      // the last 60s" — in a mass re-auth right after a token write, every
      // sibling would retire the live claim and open its own tab (the
      // issue-#10 storm again). Satisfied means tokens STRICTLY after the
      // claim. The grace belongs only to oauthSpawnGate, where a stale
      // "satisfied" merely allows a spawn whose tab is still claim-gated.
      if (newerTokensExist(serverUrl, stolen.openedAt)) return true;
 ...
 * with the small RETIRE_TOKENS_GRACE_MS (deletion-grade evidence — the 60s
 * gate grace over-deletes, see the constant), and retire only the exact
 * marker instance observed (claimId match) — a different claim that
...
function retireSatisfiedAuthMarker(serverUrl, ttlMs = AUTH_MARKER_TTL_MS) {
  ...
  // RETIRE_TOKENS_GRACE_MS, not the 60s gate grace: a deletion judged with
  // the gate's window would retire another session's LIVE claim whenever a
  // 401 follows a token write inside it (see the constant's comment).
  if (!newerTokensExist(serverUrl, observed.openedAt - RETIRE_TOKENS_GRACE_MS)) {
...
function tryClaimAuthPrompt(serverUrl, ttlMs = AUTH_MARKER_TTL_MS) {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      ...
      fs.writeFileSync(
        markerPath,
        JSON.stringify({ openedAt, kind: "opened", claimId }) + "\n",
        { flag: "wx" }
      );
    ...
    const marker = readFreshAuthMarker(serverUrl, ttlMs);
    // Strictly newer than the claim, no TOKENS_GRACE_MS — see
    // tryRetireAuthMarker (re-review P1): the graced form reads any token
    // write in the last 60s as satisfying a just-made claim.
    if (
      marker &&
      marker.kind === "opened" &&
      !newerTokensExist(serverUrl, marker.openedAt)
    ) {
