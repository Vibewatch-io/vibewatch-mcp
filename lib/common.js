 ...
 * the same rename-and-verify protocol as tryTakeoverClaim, never a
 * read-then-rm (two contenders reading one stale marker could otherwise both
 * "retire" it, the second rm-ing the first's fresh claim — the plan-review
 * P1). Retire-able: expired, satisfied (newer tokens landed), or kind
 * "wait" (no tab behind it). A stolen fresh `opened` marker is restored.
...
function tryRetireAuthMarker(serverUrl, ttlMs = AUTH_MARKER_TTL_MS) {
  const livePath = authMarkerPath(serverUrl);
  const stolenPath = `${livePath}.retire-${process.pid}-${crypto
    .randomBytes(4)
    .toString("hex")}`;
  try {
  ...
  // Verify AFTER the rename: the marker may have changed between the
  // caller's read and our steal.
  let stolen = null;
  try {
    const parsed = JSON.parse(fs.readFileSync(stolenPath, "utf8"));
  ...
  const now = Date.now();
  const fresh =
    stolen && stolen.openedAt <= now && now - stolen.openedAt <= ttlMs;
  const satisfied =
    fresh && newerTokensExist(serverUrl, stolen.openedAt - TOKENS_GRACE_MS);
  if (fresh && !satisfied && stolen.kind === "opened") {
    // We stole a live opened-tab marker — restore it only if the path is
    // still free (rename overwrites, and a third process may have claimed
    // in the steal window; same residual policy as tryTakeoverClaim).
    try {
...
function tryClaimAuthPrompt(serverUrl, ttlMs = AUTH_MARKER_TTL_MS) {
  const markerPath = authMarkerPath(serverUrl);
  // Two rounds: wx → (blocked? / retire) → wx. A second EEXIST after a
  // successful retire means another contender claimed in between — yield.
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    if (!tryRetireAuthMarker(serverUrl, ttlMs)) return { claimed: false };
  }
  return { claimed: false };
}
...
function tryTakeoverClaim(serverUrl, ownerId) {
  ...
  // LIVE claim. Restore it ONLY if the path is still free: rename
  // overwrites an existing destination on every platform, so a blind
  // rename-back would clobber a third process's fresh claim created in the
  // steal window. (existsSync→rename is itself a narrow race — the
  // residual triple-race documented on the protocol.) When the path is
  // taken, drop the stolen file; its owner notices the ownership change at
  // its next renewal and stops renewing.
  try {
    if (!fs.existsSync(livePath)) {
      fs.renameSync(stolenPath, livePath);
      return false;
