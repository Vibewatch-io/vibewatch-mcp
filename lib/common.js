...
function writeAuthMarker(serverUrl, kind = "opened") {
  ...
  try {
    fs.writeFileSync(tmpPath, JSON.stringify({ openedAt, kind }) + "\n");
 ...
 */
function demoteAuthMarkerToWait(serverUrl, expectedOpenedAt) {
  if (!Number.isFinite(expectedOpenedAt)) return;
  const markerPath = authMarkerPath(serverUrl);
  const freed = atomicRetirePath(
    ...
    (stolen) =>
      Boolean(
        stolen &&
          stolen.kind === "opened" &&
          stolen.openedAt === expectedOpenedAt
      )
  );
  if (!freed) return;
  try {
...
function tryClaimAuthPrompt(serverUrl, ttlMs = AUTH_MARKER_TTL_MS) {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      ...
      fs.writeFileSync(
        markerPath,
        JSON.stringify({ openedAt, kind: "opened" }) + "\n",
        { flag: "wx" }
      );
      return { claimed: true, openedAt };
    } catch (err) {
      if (err.code !== "EEXIST") {
  ...
  try {
    return { claimed: true, openedAt: writeAuthMarker(serverUrl, "opened") };
  } catch (err) {
    return { claimed: true, degraded: err.message };
  }
}
  ...
  for (;;) {
    ...
    if (acquired) {
      ...
      if (gateMode === "bridge") {
        ...
        // must survive a stale "satisfied" read (re-review round 2).
        if (gate === "satisfied") tryRetireAuthMarker(serverUrl);
      }
      return handle;
    }
