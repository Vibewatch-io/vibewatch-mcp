...
const CLAIM_PREAUTH_ABORT_MS = 60_000;
...
async function acquireSpawnSlot(serverUrl, opts = {}) {
  ...
  for (;;) {
    try {
      ...
      const handle = makeClaimHandle(serverUrl, filePath, ownerId, renewMs);
      if (gateMode === "bridge") {
...
function makeClaimHandle(serverUrl, filePath, ownerId, renewMs = CLAIM_RENEW_MS) {
  let lost = false;
  let released = false;
