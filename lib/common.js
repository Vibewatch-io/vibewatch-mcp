 ...
 * with the same TOKENS_GRACE_MS the gate used, but retire only the exact
...
function retireSatisfiedAuthMarker(serverUrl, ttlMs = AUTH_MARKER_TTL_MS) {
  ...
  if (!newerTokensExist(serverUrl, observed.openedAt - TOKENS_GRACE_MS)) {
    return false; // not satisfied even with the grace — leave it standing
  }
  return atomicRetirePath(
    authMarkerPath(serverUrl),
    `retire-${process.pid}-${crypto.randomBytes(4).toString("hex")}`,
 ...
 * it only when it is exactly the one this process wrote (openedAt match on
