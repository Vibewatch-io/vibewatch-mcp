...
/** Random identity a marker carries so ownership checks can't collide. */
function newClaimId() {
 ...
 * tryClaimAuthPrompt instead. Direct-form callers: tryClaimAuthPrompt's
 * documented fail-open tail, and the tests.
 */
function writeAuthMarker(serverUrl, kind = "opened") {
  ...
  // claimId is the ownership handshake for demotion and identity-matched
  // retirement — openedAt alone is not unique (two claims can share a
  // millisecond; re-review P2). tokensSeen is the satisfaction watermark
  // (see markerSatisfied).
  const claimId = newClaimId();
  const tokensSeen = latestTokenMtime(serverUrl);
  try {
    fs.writeFileSync(
      tmpPath,
      JSON.stringify({ openedAt, kind, claimId, tokensSeen }) + "\n"
    );
    fs.renameSync(tmpPath, markerPath);
    return { openedAt, claimId };
  } catch (err) {
...
function readFreshAuthMarker(serverUrl, ttlMs = AUTH_MARKER_TTL_MS) {
  try {
    ...
    return {
      ...
      tokensSeen: Number.isFinite(parsed.tokensSeen) ? parsed.tokensSeen : null,
    };
  } catch {
    return null;
  }
...
/**
 * The newest mtime across every version dir's token file for this server,
 * or 0 with no tokens. Captured into a claim as its `tokensSeen` watermark:
 * "satisfied" for that claim then means the token file CHANGED since the
 * claim was made (latest > tokensSeen) — a state comparison immune to the
 * clock-window ambiguity that made every graced/strict mtime-vs-openedAt
 * rule wrong in one direction (round 3: 60s grace let siblings retire a
 * live claim made within 60s of a token write; round 4: strict comparison
 * left coarse-mtime completions stuck; round 5: a 5s grace still let a
 * claim made 3s after a token write be retired, cascading on Windows).
 */
function latestTokenMtime(serverUrl) {
  const hash = serverHash(serverUrl);
  const base = authCacheBase();
  let versionDirs;
  try {
  ...
  // Ignore far-future mtimes (clock moved backward since the write, or a
  // restored file): baked into a claim's tokensSeen, one would make every
  // later genuine completion compare below the watermark and strand the
  // marker until the TTL (PR review). Same policy as readFreshAuthMarker's
  // future-openedAt handling; the slack absorbs normal clock jitter.
  ...
  for (const versionDir of versionDirs) {
    try {
      ...
      if (stat.mtimeMs > latest && stat.mtimeMs <= ceiling) {
        latest = stat.mtimeMs;
      }
    } catch {
      /* no tokens in this version dir */
...
function oauthSpawnGate(serverUrl, ttlMs = AUTH_MARKER_TTL_MS) {
  ...
  // `opened` markers use the same watermark predicate retirement uses —
  // when the gate said "satisfied" on graced evidence that retirement then
  // rejected, an orphaned claim let every respawn spawn, prompt, and be
  // refused the tab: a zero-tab loop until the TTL (Cubic P1). Suppressing
  // instead is the intended issue-#4 behavior for an unanswered prompt.
  if (markerSatisfied(serverUrl, marker)) return "satisfied";
  // `wait` markers carry no tab, and retirement retires them
  // unconditionally, so the graced check is safe here — it exists for the
  // lockfile-race loser whose wait marker lands moments AFTER the winner's
  // tokens (a working sign-in must not be suppressed for the TTL).
  if (
    marker.kind === "wait" &&
...
/**
 * Has the sign-in behind this `opened` marker completed? Watermark rule
 * when the marker carries tokensSeen (all markers written since round 6):
 * satisfied iff the token file changed after the claim. Legacy markers
 * fall back to strictly-newer-than-openedAt — the direction that never
 * deletes a live claim.
 */
function markerSatisfied(serverUrl, marker) {
  if (Number.isFinite(marker.tokensSeen)) {
    return latestTokenMtime(serverUrl) > marker.tokensSeen;
  }
  return newerTokensExist(serverUrl, marker.openedAt);
}
...
function readMarkerFile(filePath) {
  try {
    ...
    return {
      ...
      kind: parsed.kind === "wait" ? "wait" : "opened",
      // Absent on legacy (pre-#10) markers; identity matches then fail,
      // which fails safe (nothing gets demoted or identity-retired).
      claimId: typeof parsed.claimId === "string" ? parsed.claimId : null,
      // Token watermark at claim time; null on legacy markers, which then
      // fall back to the strict openedAt comparison.
      tokensSeen: Number.isFinite(parsed.tokensSeen) ? parsed.tokensSeen : null,
    };
  } catch {
    return null;
...
function tryRetireAuthMarker(serverUrl, ttlMs = AUTH_MARKER_TTL_MS) {
  return atomicRetirePath(
    ...
    (stolen) => {
      ...
      // markerSatisfied, not a time-window rule (re-review P1s, rounds
      // 3-6): the watermark compares token STATE against what the claim
      // observed, so neither "any token write in the last N seconds" nor
      // coarse-mtime truncation can misjudge a live claim as satisfied.
      if (markerSatisfied(serverUrl, stolen)) return true;
...
/**
 * Retirement for the "satisfied" paths (spawn gate, win32 claim recheck,
 * proxy-up): watermark-JUDGED and identity-MATCHED. The strict
 * tokens-newer-than-openedAt rule in tryRetireAuthMarker protects the
 * one-tab guarantee, but on coarse-mtime filesystems a completed sign-in's
 * token write can land in the same timestamp bucket as the claim — the
 * gate (graced) then reads satisfied while strict retirement declines
 * forever, leaving the finished marker to suppress the NEXT genuine
 * re-auth for the full TTL (re-review P1). So here: judge satisfaction
 * with the tokensSeen watermark (markerSatisfied — deletion-grade
 * evidence: the token file must have CHANGED since the claim; every
 * mtime-vs-openedAt window rule misjudged one direction, rounds 3-6), and
 * retire only the exact marker instance observed (claimId match) — a
 * different claim that
 * appears in the window is restored by the primitive, never deleted on a
 * stale judgment. Wait/stale/corrupt markers fall through to the strict
 * retire, which already handles them.
 */
function retireSatisfiedAuthMarker(serverUrl, ttlMs = AUTH_MARKER_TTL_MS) {
  const observed = readFreshAuthMarker(serverUrl, ttlMs);
  if (!observed || observed.kind !== "opened") {
    // Absent, expired-or-corrupt (readFresh → null), or `wait` — the
    // strict path retires whatever actually stands.
    return tryRetireAuthMarker(serverUrl, ttlMs);
  }
  if (!markerSatisfied(serverUrl, observed)) {
    return false; // not satisfied — leave it standing
  }
  return atomicRetirePath(
    authMarkerPath(serverUrl),
    `retire-${process.pid}-${crypto.randomBytes(4).toString("hex")}`,
    ...
    (stolen) =>
      Boolean(
        stolen &&
          // Identity: same claimId, or — legacy markers with no claimId —
          // the same openedAt (best available identity for old files).
          (observed.claimId
            ? stolen.claimId === observed.claimId
            : !stolen.claimId && stolen.openedAt === observed.openedAt)
      )
  );
}

/**
 * After a failed browser opener: the `opened` marker this process claimed
 * would otherwise suppress every session for the full TTL with NO tab
 * behind it (review P2). Demoting it to `wait` keeps host respawns
 * suppressed but lets another session's prompt claim and try ITS opener.
 *
 * Ownership-verified, never a blind overwrite (re-review P1): the opener's
 * failure can resolve arbitrarily late, by which time this phase's marker
 * may have been retired and a NEWER phase may hold the path — replacing
 * that claim with `wait` would let yet another session open a second tab.
 * The demotion steals the marker via the shared retire primitive, retires
 * it only when it is exactly the one this process wrote (claimId match on
 * an `opened` marker; anything else is restored), then writes the `wait`
 * marker exclusively — losing that wx to a new claimant means theirs
 * stands, which is correct. Best-effort throughout; the TTL backstops.
 */
function demoteAuthMarkerToWait(serverUrl, expectedClaimId) {
  if (typeof expectedClaimId !== "string" || expectedClaimId === "") return;
  const markerPath = authMarkerPath(serverUrl);
  const freed = atomicRetirePath(
    markerPath,
    `demote-${process.pid}-${crypto.randomBytes(4).toString("hex")}`,
    readMarkerFile,
    (stolen) =>
      Boolean(
        stolen &&
          stolen.kind === "opened" &&
          stolen.claimId === expectedClaimId
      )
  );
  if (!freed) return;
  try {
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        kind: "wait",
        claimId: newClaimId(),
        tokensSeen: latestTokenMtime(serverUrl),
      }) + "\n",
      { flag: "wx" }
    );
  } catch {
    /* a new claimant (or unusable storage) — leave the path as it is */
  }
}
...
function tryClaimAuthPrompt(serverUrl, ttlMs = AUTH_MARKER_TTL_MS) {
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    try {
      ...
      fs.writeFileSync(
        markerPath,
        JSON.stringify({ openedAt, kind: "opened", claimId, tokensSeen }) +
          "\n",
        { flag: "wx" }
      );
      return { claimed: true, openedAt, claimId };
    } catch (err) {
      if (err.code !== "EEXIST") {
    ...
    if (
      marker &&
      marker.kind === "opened" &&
      !markerSatisfied(serverUrl, marker)
    ) {
  ...
  // Starved past every atomic round — a `wait` writer keeps re-creating the
  // path inside our retire→create gaps. If a live tab now stands, yield;
  // otherwise fail OPEN with an overwrite. The overwrite is the one
  // non-exclusive write in the protocol, kept because a wrong
  // `claimed:false` here costs the whole sign-in (nobody else will open —
  // auto-open is suppressed). The residual is one extra tab per session
  // that BOTH starves through all five atomic rounds AND races a prompt
  // owner inside the µs between this read and the rename — each starved
  // session pays it independently, but reaching this line at all needs
  // five consecutive adverse interleavings, so the expected cost stays far
  // below the pre-#10 storm this replaces.
  const marker = readFreshAuthMarker(serverUrl, ttlMs);
  if (
    marker &&
    marker.kind === "opened" &&
  ...
  try {
    return { claimed: true, ...writeAuthMarker(serverUrl, "opened") };
  } catch (err) {
    return { claimed: true, degraded: err.message };
  }
}
 ...
 */
// How long openBrowser waits for the opener to exit before assuming it is a
// lingering hand-off (some openers stay alive beside the browser) and
// counting it a success. Platform openers that FAIL (no browser, bad URL
// handler) exit nonzero well inside this.
const OPEN_EXIT_WAIT_MS = 3_000;

/** WSL reports platform "linux" but needs the Windows-side opener. */
function isWsl(platform, env, release) {
  if (platform !== "linux") return false;
  if (env.WSL_DISTRO_NAME || env.WSL_INTEROP) return true;
  try {
    return /microsoft/i.test(release ?? os.release());
  } catch {
    return false;
  }
}
...
function openBrowser(
  url,
  { env = process.env, platform = process.platform, osRelease } = {}
) {
  return new Promise((resolve) => {
    ...
    } else if (isWsl(platform, env, osRelease)) {
      // Plain xdg-open targets a Linux desktop that a WSL install usually
      // doesn't have — wslview hands the URL to the Windows browser (the
      // suppressed `open` package makes the same distinction; PR review).
      command = "wslview";
      args = [url];
    } else {
      command = "xdg-open";
    ...
    // Success needs more than a successful spawn: an opener that starts and
    // then exits nonzero opened nothing, and reporting true would leave an
    // `opened` marker suppressing every session for the TTL with no tab
    // (PR review). Wait briefly for the exit; a clean exit or a lingering
    // hand-off counts as opened, a nonzero exit does not.
    child.once("exit", (code) => settle(code === 0));
    child.once("spawn", () => {
      child.unref();
      // Deliberately NOT unref'd: with nothing else on the event loop an
      // unref'd timer lets the process exit before this promise settles,
      // so an opener failure would never demote the `opened` claim (PR
      // review). Worst case it holds exit for OPEN_EXIT_WAIT_MS.
      exitTimer = setTimeout(() => settle(true), OPEN_EXIT_WAIT_MS);
    });
  });
}
  ...
  for (;;) {
    ...
    if (acquired) {
      ...
      if (gateMode === "bridge") {
        ...
        // Retire, never blind-clear — a newer phase's fresh `opened` claim
        // must survive a stale "satisfied" read (re-review round 2); the
        // satisfied variant is watermark-judged so a coarse-mtime completed
        // marker retires while a live claim never reads satisfied.
        if (gate === "satisfied") retireSatisfiedAuthMarker(serverUrl);
      }
      return handle;
    }
