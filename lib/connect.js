...
async function run(argv) {
  ...
  try {
    ...
    // one-prompt-per-missing-sign-in cap for this server. Retire, never
    // blind-clear: a newer auth phase's fresh `opened` claim on the same
    // path must survive (the tokens this run just landed make our own
    // phase's marker read satisfied, so it retires).
    if (!opts.key) tryRetireAuthMarker(opts.url);
    process.stderr.write(
      signedIn
