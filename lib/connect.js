...
function resetCachedAuth(serverUrl) {
  ...
  try {
    ...
    // STALE claims only: a live claim is another process mid-sign-in, and
    // deleting it would free this reset to open a second tab beside that
    // session's — the exact race the claim exists to prevent. Retirement
    // goes through tryTakeoverClaim (atomic rename + verify-after-rename),
    // never a read-then-rm, which could delete a live successor claim that
    // appeared between the check and the removal. Unlike the marker, the
    // claim does NOT count toward `removed`: it isn't a cached sign-in,
    // and counting it would print "Cleared the cached sign-in" when
    // nothing the user asked about existed.
    if (claimIsStale(readClaimFile(claimPath(serverUrl)))) {
      tryTakeoverClaim(serverUrl, `reset-${process.pid}`);
    }
  } catch (err) {
...
async function verifyAuth(opts) {
  ...
  if (!opts.key && spawnClaimEnabled()) {
    ...
    } else if (slot.status === "unclaimed") {
      // Same fail-open policy as the bridge — but say it, or a racing
      // session on unusable claim storage double-prompts with no signal.
      process.stderr.write(
        `vibewatch-mcp: could not record the sign-in claim (${slot.reason}) ` +
          "— a concurrent session may open its own sign-in tab.\n"
      );
    }
  }
  try {
...
async function run(argv) {
  ...
  try {
    ...
    // one-prompt-per-missing-sign-in cap for this server. UNCONDITIONAL
    // clear, unlike the bridge's own retire-based paths: on the
    // already-signed-in path this run writes no new tokens, so a
    // conditional retire would leave a spurious `opened` marker (e.g. from
    // a transient-401 prompt) standing and connect-buzz — the recovery the
    // suppress message points at — would be a no-op for the full 24h TTL
    // (re-review P1). connect-buzz is an explicit user action; if it races
    // a genuinely newer phase, the cost is that phase re-prompting once.
