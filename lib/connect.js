    ...
    const splitter = makeLineSplitter((rawLine) => {
      ...
      }
      {
        if (settled) {
          ...
          // Printed by the auth owner AND by a secondary instance waiting on
          // another process's sign-in lock — the secondary never prints the
          // authorize-URL prompt itself, so the consent window must be
          // armed here or the 30s reach-phase timer kills a valid shared
          // sign-in mid-approval.
          armTimer(
            AUTH_WAIT_MS,
...
// the bare name: GUI apps inherit the full registry PATH there, and an
// absolute path under a username with spaces couldn't ride through cmd.exe.
function bridgeCommand(opts) {
  if (process.platform === "win32") return "vibewatch-mcp";
