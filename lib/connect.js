...
function resetCachedAuth(serverUrl) {
  ...
  try {
    ...
    // session's — the exact race the claim exists to prevent. Unlike the
    // marker, the claim does NOT count toward `removed`: it isn't a cached
    // sign-in, and counting it would print "Cleared the cached sign-in"
    // when nothing the user asked about existed.
    if (claimIsStale(readClaimFile(claimPath(serverUrl)))) {
      fs.rmSync(claimPath(serverUrl), { force: true });
    }
  } catch (err) {
