...
if (process.env.VIBEWATCH_MCP_SUPPRESS_BROWSER_OPEN === "1") {
  ...
  childProcess.spawn = function spawn(command, args, options) {
    ...
    // wait-mode close listener behave as with a real opener — except
    // windowsVerbatimArguments: the `open` package sets it for PowerShell's
    // sake, and verbatim (unquoted) joining would hand node a bare `-e`
    // with no value, making the no-op child exit non-zero on win32 and
    // breaking open()'s wait-mode success contract. `shell` likewise must
    // not reinterpret the replacement argv.
    const replacementOpts = { ...(opts || {}) };
    delete replacementOpts.windowsVerbatimArguments;
    delete replacementOpts.shell;
    return realSpawn.call(this, process.execPath, ["-e", ""], replacementOpts);
  };
}
